...
function Capsule( acc,coll,spec ) {
  var self = {}
...
function DataCapsule( opt ) {
  ...
  self.capsule = function( acc,coll,spec, cb ) {
    ...
    if( !cap ) {
      cap = self.capsulemap[capname] = new Capsule(acc,coll,spec)
    }
    cb(null,cap)
  }
  ...
  self.api.sync = {
    ...
    get_updates: getcap(function(req,res,cap,version){
      ...
      console.log('history from '+version+': '+JSON.stringify(history))
      ...
      console.log('itemacts: '+JSON.stringify(itemacts))
      ...
      function getitem(i) {
        if( i < itemacts.length ) {
          ...
          util.sendjson(res,{update:items})
        }
      }
      getitem(0)
    }),
